...
import { createReminderSeed, createUserSeed, withRollbackTx } from "../lib/testing.ts";
...
describe("reminderLoader integration", () => {
  ...
  it("loadCollection returns only reminders for requested user", async () => {
    await withRollbackTx(async (tx) => {
      ...
      const ownerFirst = createReminderSeed(owner.id, {
        title: "Owner earlier",
        nextAlertDate: new Date("2026-05-01T00:00:00.000Z"),
        reminderDate: new Date("2026-06-01T00:00:00.000Z"),
      });
      const ownerSecond = createReminderSeed(owner.id, {
        title: "Owner later",
        nextAlertDate: new Date("2026-05-02T00:00:00.000Z"),
        reminderDate: new Date("2026-06-01T00:00:00.000Z"),
      });
      const foreignReminder = createReminderSeed(otherUser.id, {
        title: "Foreign",
        nextAlertDate: new Date("2026-05-01T00:00:00.000Z"),
        reminderDate: new Date("2026-06-01T00:00:00.000Z"),
      });

      await tx.insert(reminder).values([ownerSecond, ownerFirst, foreignReminder]);
